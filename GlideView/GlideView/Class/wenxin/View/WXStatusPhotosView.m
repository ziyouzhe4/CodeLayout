...
#import "WXStatusPhotoView.h"
#import "MJPhotoBrowser.h"
#import "MJPhoto.h"
#import "WXPhoto.h"

@implementation WXStatusPhotosView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        //预先开辟9个存储空间,避免每次都申请
        for (int i = 0; i< 9; i++) {
            WXStatusPhotoView *photoView = [[WXStatusPhotoView alloc] init];
            photoView.tag = i;
            ...
            [recognizer addTarget:self action:@selector(tapPhotos:)];
 ...
 *  监听图片的点击
 */
- (void)tapPhotos:(UITapGestureRecognizer *)recognizer
{
...
- (void)setPic_urls:(NSArray *)pic_urls
{
    ...
    for(int i=0 ;i < self.subviews.count;i++){
        WXStatusPhotoView *photoView = self.subviews[i];
        if (i<pic_urls.count) {
            photoView.hidden = NO;
